...
document.getElementById("boton-realizar-pago-carrito").addEventListener("click", mostrarDialogoFormaPago);

function mostrarDialogoFormaPago() {
  const dialogoFormaPago = document.getElementById("dialogoFormaPago");
  if (!dialogoFormaPago.showModal) {
    dialogPolyfill.registerDialog(dialogoFormaPago);
  }

  dialogoFormaPago.showModal();

  document.getElementById("boton-pago-tarjeta").addEventListener("click", pagarConTarjeta);
}
